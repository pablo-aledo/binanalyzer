...
class RelevantStatementsTest extends FunSuite with TableDrivenPropertyChecks with Matchers with PredefinedParsers {

    override def suiteName = "Slicing"
    ...
    test( s"Check RelevantStatements" ) {
        slice( nfa, finalNode, Set( VarReg( R14( "lr" ) ) ), p ) shouldBe Set(
            UniqueNode( 65568, List( 65560 ) ),
            UniqueNode( 65556, List( 65568 ) )
        )
